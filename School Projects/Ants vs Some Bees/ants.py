...
class Insect:
    ...
    def __init__(self, armor, place=None):
        ...
        self.armor = armor
        self.place = place
        self.scared = False  # set by Place.add_insect and Place.remove_insect
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        bees_place = self.place
        ent_transition = 0
        while bees_place.name != str(beehive):
        	if len(bees_place.bees) != 0:
        		if ent_transition >= self.min_range and ent_transition <= self.max_range:
        			return random_or_none(bees_place.bees)
        	ent_transition += 1
        	bees_place = bees_place.entrance
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor - amount <= 0:
        	total_bees = list(self.place.bees)
        	for b in total_bees:
        		b.reduce_armor(self.damage)
        		if self.place == b.place:
        			b.reduce_armor(b.damage)
        else:
        	total_bees = list(self.place.bees)
        	for b in total_bees:
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.imposter:
            ...
            while place.exit:
                ant = place.exit.ant
                if ant and ant not in self.double_ants:
                    ant.damage *= 2
                    self.double_ants.append(ant)
                if isinstance(ant, ContainerAnt) and ant and ant.contained_ant:
                    if ant.contained_ant not in self.double_ants:
                        ant.contained_ant.damage *= 2 
                        self.double_ants.append(ant.contained_ant)
                place = place.exit
...
def apply_effect(effect, bee, duration):
    ...
    "*** YOUR CODE HERE ***"
    regular = bee.action
    special_effect = effect(bee.action, bee)

    def turning_rounds(gamestate):
        nonlocal duration
        if duration != 0:
            duration -= 1
            return special_effect(gamestate)
        else:
            bee.scared = False
            return regular(gamestate)
            
    bee.action = turning_rounds
